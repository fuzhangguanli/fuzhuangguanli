...
private:
	char *m_name;
public:
	static pj_pool_t *m_pool;
public:
	name():m_name(NULL) {}

	~name() { free(m_name);}
	static void init();
	void setname(const char * n) { m_name = (char *)malloc(strlen(n)+1); memcpy(m_name,n,strlen(n)+1);}
	const char * getname(){ return m_name;}
};

class PMutex : public name
{
private:
	pj_mutex_t *m_mutexid;	
public:
	PMutex();
	~PMutex();
	int lock();
...
private:
	...
	PWaitAndSignal();
public:
	PWaitAndSignal(PMutex &mutex):m_mutex(mutex){ m_mutex.lock();}
	~PWaitAndSignal(){ m_mutex.unlock();}
};
